...
from util.dataset import PartitionedDataset


def data_partition(
    layers: nn.Sequential, dataset: VisionDataset, partition_num: int, output_file=None
) -> list[Dataset]:
    """
    ...
    for output_range in partition_range(output_height, output_width, partition_num):
        # [start, end)
        ...
        dataset_tqdm = tqdm(
            dataset, desc=f"Partitioning dataset, input_range is {input_range}"
        )
        for img, _ in dataset_tqdm:
            ...
            conv_segment_output = layers(img.unsqueeze(0)).squeeze(0)
            conv_segment_labels.append(conv_segment_output)

        images_list.append(torch.stack(images, dim=0))

    labels = [label for _, label in dataset]
    partitioned_dataset = PartitionedDataset(
        images_list=images_list, conv_segment_labels=conv_segment_labels, labels=labels
    )
    ...
    if output_file is not None:
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        torch.save(partitioned_dataset, output_file)
