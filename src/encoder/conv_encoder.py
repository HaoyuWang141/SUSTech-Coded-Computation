...
class CatChannelConvEncoder(Encoder):
    def __init__(self, num_in: int, num_out: int, in_dim: tuple, intermediate_channels=20):
        ...
        self.nn = nn.Sequential(
            nn.Conv2d(in_channels=num_in * in_dim[0], out_channels=int_channels,
            ...
            nn.Conv2d(in_channels=int_channels, out_channels=int_channels,
                      kernel_size=3, stride=1, padding=1, dilation=1),
            nn.ReLU(),
            nn.Conv2d(in_channels=int_channels, out_channels=num_out * in_dim[0],
    ...
    def forward(self, datasets: list[torch.Tensor]) -> list[torch.Tensor]:
        ...
        datasets = datasets.view(batch_size, self.num_in * self.in_dim[0],
                                 self.in_dim[1], self.in_dim[2])
        out = self.nn(datasets)
        out = out.view(batch_size, self.num_out, self.out_dim[0], self.out_dim[1], self.out_dim[2])
...
class CatBatchSizeConvEncoder(Encoder):
    def __init__(self, num_in: int, num_out: int, in_dim: tuple, intermediate_channels=20):
        ...
        int_channels = intermediate_channels * num_in
        self.nn = nn.Sequential(
            nn.Conv2d(in_channels=num_in, out_channels=int_channels,
                      kernel_size=3, stride=1, padding=1, dilation=1),
            nn.ReLU(),
            nn.Conv2d(in_channels=int_channels, out_channels=int_channels,
                      kernel_size=3, stride=1, padding=1, dilation=1),
            ...
            nn.Conv2d(in_channels=int_channels, out_channels=int_channels,
                      kernel_size=3, stride=1, padding=1, dilation=1),
            nn.ReLU(),
            nn.Conv2d(in_channels=int_channels, out_channels=num_out,
                      kernel_size=1, stride=1, padding=0, dilation=1)
        )
